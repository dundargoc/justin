...
Project-local `CLAUDE.md` and per-project memory may refine/override these.
...
- **Never mutate git history without an explicit ask each turn** — no `git commit`, `push`,
  `rebase`, `reset`, amend, or force-push, even if approved in a prior turn. Workspace
  edits (Edit/Write to tracked files) are fine.
